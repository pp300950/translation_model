...
# ฟังก์ชันสำหรับโหลดข้อมูลการแปลจากไฟล์
def load_translation_data(filename):
    ...
    reverse_translation_dict_th_zh = create_reverse_translation_dict(translation_dict, 'zh')
    reverse_translation_dict_en_th = create_reverse_translation_dict(translation_dict, 'en')
    reverse_translation_dict_th_en = create_reverse_translation_dict_reverse(translation_dict, 'th')

    return translation_dict, example_sentences, reverse_translation_dict_th_zh, reverse_translation_dict_en_th, reverse_translation_dict_th_en

# ฟังก์ชันสำหรับสร้าง reverse translation dictionary
def create_reverse_translation_dict(translation_dict, target_lang):
    reverse_translation_dict = {}
    for k, v in translation_dict.items():
        if isinstance(v, dict):
            if target_lang in v:
                inner_v = v[target_lang]
...
# ฟังก์ชันสำหรับสร้าง reverse translation dictionary แบบกลับ
def create_reverse_translation_dict_reverse(translation_dict, source_lang):
    reverse_translation_dict = {}
    for k, v in translation_dict.items():
        if isinstance(v, dict):
            if source_lang in v:
                inner_v = v[source_lang]
                if isinstance(inner_v, list):
                    for item in inner_v:
                        reverse_translation_dict[k] = item
                else:
                    reverse_translation_dict[k] = inner_v
        else:
            reverse_translation_dict[k] = v
    return reverse_translation_dict

# ฟังก์ชัน softmax สำหรับการคำนวณ attention weights
def softmax(x):
    e_x = np.exp(x - np.max(x))
    return e_x / e_x.sum(axis=-1, keepdims=True)

# ฟังก์ชันสำหรับคำนวณ attention weights
def attention_weights(decoder_hidden, encoder_outputs):
    scores = np.dot(encoder_outputs, decoder_hidden)
    weights = softmax(scores)
    return weights

# ฟังก์ชันสำหรับคำนวณ context vector
def context_vector(attention_weights, encoder_outputs):
    context = np.sum(attention_weights[:, np.newaxis] * encoder_outputs, axis=0)
    return context

# ฟังก์ชันสำหรับแปลประโยคด้วย attention mechanism
def translate_with_attention(source_words, translation_dict, attention_weights, context_vector):
    ...
    translated_sentence = ' '.join(map(str, translated_words))
    return translated_sentence

# ฟังก์ชันสำหรับ tokenizing ประโยค
...
# ฟังก์ชันสำหรับแปลประโยค
def translate_sentence(source_sentence, direction, translation_dict, reverse_translation_dict_th_zh, reverse_translation_dict_en_th, reverse_translation_dict_th_en):
    ...
    if direction == 'zh-th':
        ...
        tokenized_source = tokenize_sentence(source_sentence, reverse_translation_dict_th_zh)
        return translate_with_attention(tokenized_source.split(), reverse_translation_dict_th_zh, weights, context)
    elif direction == 'th-en':
        tokenized_source = tokenize_sentence(source_sentence, reverse_translation_dict_th_en)
        return translate_with_attention(tokenized_source.split(), reverse_translation_dict_th_en, weights, context)
...
if __name__ == "__main__":
    ...
    translation_dict, example_sentences, reverse_translation_dict_th_zh, reverse_translation_dict_en_th, reverse_translation_dict_th_en = load_translation_data(filename)
    ...
    for example in example_sentences:
        ...
        for chinese_word, thai_word, english_word in zip(chinese_tokens, thai_tokens, english_tokens):
            ...
            if thai_word not in reverse_translation_dict_th_zh:
                reverse_translation_dict_th_zh[thai_word] = chinese_word
            if english_word not in reverse_translation_dict_en_th:
                reverse_translation_dict_en_th[english_word] = thai_word
            if thai_word not in reverse_translation_dict_th_en:
                reverse_translation_dict_th_en[thai_word] = english_word

    while True:
        direction = input("zh-th,th-zh,th-en,en-th: ")
        source_sentence = input("Enter: ")

        translated_sentence = translate_sentence(source_sentence, direction, translation_dict, reverse_translation_dict_th_zh, reverse_translation_dict_en_th, reverse_translation_dict_th_en)
